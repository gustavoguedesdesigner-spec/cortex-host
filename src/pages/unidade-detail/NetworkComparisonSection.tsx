        ...
        <div className="flex flex-col divide-y divide-border-subtle">
          {rows.map((row) => (
            <div key={row.metrica} className="flex items-center justify-between gap-4 px-4 py-3">
              <span className="text-support text-content-secondary">{row.metrica}</span>
              <span className="flex items-center gap-3 text-support">
                <span className={cn('font-semibold', row.unidadeMelhor ? 'text-status-success' : 'text-status-critical')}>{row.unidade}</span>
                <span className="text-content-tertiary">vs. {row.rede} (rede)</span>
              </span>
            </div>
          ))}
        </div>
        {insight && <p className="text-support text-content-secondary leading-relaxed p-4 border-t border-border-subtle">{insight}</p>}
      </Card>
    </section>
  )
}
