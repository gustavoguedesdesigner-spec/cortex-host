...
export function InternalTabs({ active, onChange }: { active: UnitTab; onChange: (tab: UnitTab) => void }) {
  return (
    <div className="flex items-center gap-1 overflow-x-auto scrollbar-none border-b border-border-subtle -mx-1 px-1">
      {tabs.map((tab) => (
        <button
          key={tab}
          ...
          className={cn(
            ...
            active === tab ? 'text-content-primary' : 'text-content-tertiary hover:text-content-secondary',
          )}
        >
          {tab}
          {active === tab && <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-cortex-500 rounded-full" />}
        </button>
      ))}
    </div>
  )
...
export function UnitTabPlaceholder({ unitName, tab }: { unitName: string; tab: UnitTab }) {
  return (
    <div className="flex flex-col gap-3 py-10 items-center text-center">
      <Construction className="h-5 w-5 text-cortex-500" />
      <p className="text-card-title text-content-primary">{tab} de {unitName}</p>
      <p className="text-support text-content-tertiary max-w-md">
