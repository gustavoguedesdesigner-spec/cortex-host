...
export function TrendSection({ data, meta, eventos }: TrendSectionProps) {
  return (
    <Card>
      <p className="text-card-title text-content-primary mb-1">Tendência das últimas 8 semanas</p>
      <p className="text-support text-content-tertiary mb-3">CMV teórico, CMV real e meta — eventos observados no período</p>
        ...
        <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-border-subtle">
          {eventos.map((e, i) => (
            <Tooltip key={i} content={`${data[e.semanaIndex]?.semana ?? ''}: pode estar relacionado ao aumento do CMV — exige validação.`}>
              <span className="rounded-full bg-surface-3 border border-border-subtle px-2.5 py-1 text-caption text-content-tertiary cursor-help">
